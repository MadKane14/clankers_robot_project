...
class MissionPlannerNode(Node):
    ...
    def send_nav_goal(self, target_node_name):
        ...
        # --- NEW: Evasion Maneuver Logic ---
        if self.failed_attempts > 0:
            step_distance = 1.0 # Shorten the step to sneak out of tight spots
            # Alternate stepping left or right by 45 degrees
            angle_offset = math.radians(45) * (1 if self.failed_attempts % 2 == 0 else -1)
            self.get_logger().warn(f"Path blocked! Executing evasion maneuver (Attempt {self.failed_attempts})...")
            
            waypoint_angle = math.atan2(safe_y - self.current_y, safe_x - self.current_x) + angle_offset
            target_x = self.current_x + step_distance * math.cos(waypoint_angle)
            target_y = self.current_y + step_distance * math.sin(waypoint_angle)
            is_waypoint = True
            
        elif dist_to_safe_target > step_distance:
            waypoint_angle = math.atan2(safe_y - self.current_y, safe_x - self.current_x)
            target_x = self.current_x + step_distance * math.cos(waypoint_angle)
            target_y = self.current_y + step_distance * math.sin(waypoint_angle)
            is_waypoint = True
    ...
    def get_result_callback(self, future, target_node_name, is_waypoint):
        ...
        if result == 4: # SUCCEEDED
            ...
            self.failed_attempts += 1 # Trigger the evasion maneuver for the next loop
